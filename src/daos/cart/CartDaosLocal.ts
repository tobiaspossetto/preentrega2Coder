...
export default class CartDaosLocal extends ContenedorLocal{
    ...
    createCart(){
        try {
            ...
            this.model.push({timestamp: new Date().toLocaleDateString(), products: [], id:id});
    ...
    addProductToCart(id:string, idProd:any){
        try {
          ...
          //  console.log(prod)
            ...
            let CartUpdated = this.model.find((item: { id: string; }) => item.id == id);
    ...
    deleteProductFromCart(cartId:string,productId:string){
        try{
            ...
            Cart.products = Cart.products.filter((item: { id: string; }) => item.id != productId);
            
            let cartUpdated = {...this.model.filter((item: { id: string; }) => item.id != cartId), Cart};
