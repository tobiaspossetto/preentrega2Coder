...
export default class CarritoDaosMongo extends ContenedorMongo {
    ...
    async createCart() {
        try {
            ...
            //@ts-ignore
            const cartSavedModel = new this.model({timestamp: new Date().toLocaleDateString(), products: []});
   ...
   async addProductToCart(id:string, idProd:any) {
        try {
            ...
            let CartUpdated = await this.model.findByIdAndUpdate(id,
                Cart
            )
...
async deleteProductFromCart(idCart:any, idProduct:any) {
    try {
        ...
        console.log('cartaFinal ' ,CartFinal)
        console.log(' -------')
        let CartUpdated = await this.model.findByIdAndUpdate(
           idCart,
            Cart
        )
