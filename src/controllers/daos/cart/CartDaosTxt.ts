...
export default class CartDAosLocal extends ContenedorLocal {
  ...
  async listProductsOfCart(cartId: any) {
    try {
      let carts: any = await fs.promises.readFile(this.path);
      carts = JSON.parse(carts);
      let cart = carts.filter((cart: { id: any }) => cart.id == cartId);
  ...
  async getCart(id: string) {
    try {
      let carts: any = await fs.promises.readFile(this.path);
      carts = JSON.parse(carts);
      let cart = carts.filter((cart: { id: any }) => cart.id == id);
      return { status: 1, data: cart[0] };
    } catch (error) {
      console.log(error);
      //opte por -1 para responder con un 500
      return { status: -1 };
  ...
  async addProductToCart(idCart: string, idProduct: any) {
    try {
      let prod = await productsController.listOne(idProduct);
      if (prod.status == -1) {
        return { status: -1 };
      } else {
        let carts: any = await fs.promises.readFile(this.path);
  ...
  async deleteProductFromCart(idCart: string, idProduct: any) {
    try {
      let carts: any = await fs.promises.readFile(this.path);
      carts = JSON.parse(carts);
      ...
      cart[0].productos = cart[0].productos.filter(
        (product: { id: any }) => product.id != idProduct
      );
